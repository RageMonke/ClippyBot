...
function renderGridPNG({ weekStartISO, blocks, members, hours = { start: 8, end: 22 } }) {
  ...
  for (let d = 0; d < 7; d++) {
    ...
    ctx.fillText(DAYS[d], x + 8, headerH - 10);
  }
...
function buildBlocks({ users, userEvents, hours = { start:8, end:22 } }) {
  // 1) group by (day, start, end, title)
  ...
  for (const u of users) {
    ...
    for (const ev of evs) {
      // per-day split (in case an event crosses midnight)
      const s = dayjs(ev.start);
      const e = dayjs(ev.end);
      // split by day boundaries for clean grid painting
      let cur = s;
      while (cur.isBefore(e)) {
        ...
        if (dayjs(clipped.end).isAfter(clipped.start)) {
          ...
          if (!groups.has(key)) groups.set(key, { dayIndex, startDate: clipped.start, endDate: clipped.end, title: ev.summary, attendees: new Set() });
          groups.get(key).attendees.add(initials(u.displayName || u.username || 'User'));
        }
  ...
  for (const g of groups.values()) {
    blocks.push({
      ...
      endDate: g.endDate,
      title: g.title || 'Lesson',
      attendees: Array.from(g.attendees),
      color: hashColor(g.title || 'Lesson'),
    });
  }
...
module.exports = { renderGridPNG, buildBlocks, initials, hashColor };
