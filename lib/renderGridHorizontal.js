...
function lanePackByUserColoring(blocksDay, ownersOrder) {
  const idx = new Map(ownersOrder.map((lab, i) => [lab, i]));
  const events = [...blocksDay].sort((a, b) => {
    const oa = idx.get((Array.isArray(a.attendees) && a.attendees[0]) || '') ?? 999;
    const ob = idx.get((Array.isArray(b.attendees) && b.attendees[0]) || '') ?? 999;
    if (oa !== ob) return oa - ob;

    // Enhanced sorting for Monday and Tuesday
    ...
    const sa = a.startDate.getTime(), sb = b.startDate.getTime();
  ...
  for (const ev of events) {
    ...
    const sorted = [...atts].sort((x, y) => (idx.get(x) ?? 999) - (idx.get(y) ?? 999));
    ev._owner = sorted[0] ?? null;
  }

  const lanes = [];
  const ownerLane = new Map();
  const overlaps = (aStart, aEnd, bStart, bEnd) => aStart < bEnd - 1 && bStart < aEnd - 1;

  function laneAccepts(laneEvents, s, e) {
    for (const ev of laneEvents) {
      if (overlaps(ev.startDate.getTime(), ev.endDate.getTime(), s, e)) return false;
    }
    return true;
  }
  ...
  for (const ev of events) {
    ...
    let placed = false;
    const owner = ev._owner;

    // try owner's preferred lane (but check all events in that lane)
    if (owner && ownerLane.has(owner)) {
      const prefer = ownerLane.get(owner);
      const laneEvents = lanes[prefer];
      if (laneEvents && laneAccepts(laneEvents, s, e)) {
        ev.lane = prefer;
        lanes[prefer].push(ev);
        placed = true;
      }
    }

    if (!placed) {
      // try any existing lane (check full lane for overlaps)
      for (let li = 0; li < lanes.length; li++) {
        const laneEvents = lanes[li];
        if (laneAccepts(laneEvents, s, e)) {
          ev.lane = li;
          lanes[li].push(ev);
          placed = true;
          if (owner && !ownerLane.has(owner)) ownerLane.set(owner, li);
          break;
        }
      }
    }
  ...
  for (const ev of events) ev.lanes = lanesCount;
  return events;
}
...
function renderGridPNGHorizontal({
  ...
  for (let d = 0; d < ROW_DAYS; d++) {
    ...
    for (const b of packed) {
      ...
      const maxTextW = Math.max(12, w - (innerX - x) - 10);
      ...
      // Always draw time below title if room
      ctx.fillStyle = '#94a3b8';
  ...
  if (userLabels.length) {
    ...
    const chipW2 = 14, chipH2 = 14, gapX2 = 12, gapY2 = 8;
    let lx = 70, ly = legendTop + 2;

    for (const lbl of userLabels) {
      const col = colorFor(lbl);
      const needed = chipW2 + 6 + ctx.measureText(lbl).width + gapX2;
      if (lx + needed > width - 10) { lx = 70; ly += chipH2 + gapY2; }
      ctx.fillStyle = col; ctx.fillRect(lx, ly - chipH2 + 2, chipW2, chipH2);
      ctx.fillStyle = '#e2e8f0'; ctx.fillText(lbl, lx + chipW2 + 6, ly + 2);
      lx += needed;
    }
  }
